...
build:
  ...
  hooks:
    ...
    # 二进制合并脚本（可选，用于Zynq/Versal等SoC启动镜像生成）
    # 执行时机：比特流生成后，构建完成前
...
| `bin_merge_script` | 比特流生成后，构建完成前 | 为Zynq等SoC生成boot.bin文件    |

**关于 `bin_merge_script` 的特别说明：**

1. **可选性**：此钩子为可选配置，仅在需要为SoC设备生成启动镜像时使用
2. **执行时机**：在比特流文件生成后自动执行，用于将FSBL、比特流、U-Boot等组件合并为boot.bin
3. **默认行为**：如果未配置此钩子，构建流程将跳过二进制合并步骤
4. **脚本功能**：`scripts/merge_bin.py` 提供两种合并方式：
   - 使用Xilinx bootgen工具（推荐）
   - 简单二进制拼接（备用方案）
